...
class PPEG(nn.Module):
    """Position Encoding via depthwise CNN（三尺度 DWConv 相加）"""
    ...
    def forward(self, x: torch.Tensor, H: int, W: int) -> torch.Tensor:
        # x: [B, 1+N, C]，第 0 位是 cls_token；N = H*W
        B, L, C = x.shape
...
class TransMIL(nn.Module):
    ...
    输入:
      x: [N,D] 或 [1,N,D]（单 bag）
      coords: 可选 [N,2] / [1,N,2]（像素/微米坐标，仅取前两列）
    输出:
      logits: [1,C] 或 [C]（未激活）
    """
    def __init__(
    ...
    def _pad_to_square_seq(self, h: torch.Tensor) -> tuple[torch.Tensor, int, int]:
        """
        无 coords 时，把序列 pad 到最近的正方形长度（复制前段填充）。
        输入 h: [B,N,C] → 输出: [B,N_pad,C]，返回 H,W（H=W=sqrt(N_pad)）
        """
        B, N, C = h.shape
    ...
    def _grid_from_coords(coords: torch.Tensor) -> tuple[torch.Tensor, int, int]:
        """
        coords: [N,2] → 返回 (flat_idx[Sx*Sy], Sx, Sy)
        flat_idx 里为原序号，缺洞位置为 -1。
        """
        # 唯一化并拿到每个点的离散索引
    ...
    def _arrange_with_coords(self, h: torch.Tensor, coords: torch.Tensor) -> tuple[torch.Tensor, int, int]:
        """
        按真实坐标把实例重排成 H×W 网格并补洞到满网格。
        h: [B,N,C], coords: [N,2] → 返回 [B,H*W,C], H, W
        """
        B, N, C = h.shape
        flat_idx, H, W = self._grid_from_coords(coords)         # [H*W]
        valid = flat_idx >= 0
        # 预分配满网格
        h_pad = h.new_zeros((B, H * W, C))
        if valid.any():
            src = flat_idx[valid]                               # [M]
            h_pad[:, valid] = h[:, src, :]                      # 有效格点直接放
        # 对剩余洞位，用前若干 token 循环填充（与 pad 复制前段风格一致）
        missing = (~valid).nonzero(as_tuple=False).squeeze(1)   # [K]
    ...
    def forward(self, x: torch.Tensor, coords: torch.Tensor | None = None):
        ...
        # 2) 用 coords 排列成 H×W；无 coords 则 √N→正方形
        if coords is not None:
        ...
        logits = self.fc_out(h_cls)                              # [B,C] 或 [B,1]
